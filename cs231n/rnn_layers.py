...
#     print('Wx.shape:{}'.format(Wx.shape))
    ...
    dd=1-np.tanh(x@Wx+prev_h@Wh+b)**2
    dprev_h=np.dot(dd*dnext_h,Wh.T)
    
    dx=np.dot(dd*dnext_h,Wx.T)
    dWx=np.dot(x.T,dd*dnext_h)
    dWh=np.dot(prev_h.T,dd*dnext_h)
    db=(dnext_h*dd).sum(axis=0)
...
#         print('Wh.shape:{}'.format(Wh.shape))
        ...
        last_h,temp_cache=rnn_step_forward(x[:,i,:], prev_h, Wx, Wh, b)
        prev_h=last_h
...
#     cache=(x[:,T-1,:],h[:,T-1,:],Wx,Wh,b)    
    
#     for i in range(T-1,0,-1):
#         cache=(x[:,i-1,:],h[:,i-1,:],Wx,Wh,b)
#         dx[:,i-1,:], dprev_h, dWx, dWh, db=rnn_step_backward(dh[:,i-1,:]+dprev_h, cache)
#         print('dprev_h:{}'.format(dprev_h))
    ...
    dx = np.zeros([N,T,D])
    dx[:,T-1,:] = dx_t
    #     dxl, dprev_h, dWx, dWh, db = rnn_step_backward(dh[:,T-1,:], cache)

    for i in range(T-2, -1, -1):
        dxx, dprev_hh, dWxx, dWhh, dbb = rnn_step_backward(dh[:,i,:]+dprev_h, cache[i])
        dx[:,i,:] += dxx
        dprev_h = dprev_hh
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    pre=np.dot(prev_h, Wh)+ np.dot(x, Wx)+ b
