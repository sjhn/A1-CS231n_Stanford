...
class CaptioningRNN(object):
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
            ...
            pre_distribution=next_h@W_vocab+b_vocab
            
            pre_distribution=pre_distribution[0]
            
            p=np.exp(pre_distribution)/np.sum(np.exp(pre_distribution))
            #print(p.shape)
            sampled_index=np.random.choice(V,1,p=p)
            captions[:,i] = sampled_index
            
            x = W_embed[sampled_index]
            h0 = next_h
